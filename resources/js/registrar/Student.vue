<template>
  <div class="row justify-content-center mb-5" data-app>
          ...
          <div class="col-12 py-0">
            <div class="input-group mb-4">
              ...
              <input
                ...
                type="date"
                :class="{ 'is-invalid': form.errors.get('birthdate') }"
                class="form-control"
    ...
    <div class="col-xl-6 col-md-4">
      <blockquote class="bg-transparent">
        <h1 class="display-4 text-dark" style="font-size: 2rem">
          Encode Students Using CSV File
        </h1>
      </blockquote>
      <form action="">
            ...
            <template v-slot:default>
              ...
              <tbody>
                <tr v-for="(student, i) in students">
                  ...
                  <td>
                    <span v-if="student.status == 1" class="badge badge-success"
                      >Active</span
                    >
                  ...
                  <td class="project-actions text-right">
                    <v-btn
                      x-small
                      class="bg-primary"
                      :href="/studentProfile/ + student.student_id"
                    >
                    ...
                    <v-btn
                      x-small
                      class="bg-success"
                      @click="handleEditDialog(student)"
                    >
                    ...
                    <v-btn
                      x-small
                      class="bg-warning"
                      @click="updateUserStatus(student.user_id)"
                    >
                      <i class="fas fa-exclamation mr-1"> </i>
                      Suspend
                    </v-btn>
                    <v-btn x-small @click="showResetPassword(student.user_id)">
                    ...
                    <v-btn
                      x-small
                      class="bg-danger"
                      @click="handleDeleteDialog(student.user_id)"
                    >
    ...
    <!--    Question Prompt -->
    <v-dialog v-model="questionDialog.state" persistent max-width="400">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="questionDialog.handler">
            Yes
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="duplicatedDialog.state" persistent max-width="680">
      <v-card>
        <v-card-title class="headline"> Duplicate Entries Found </v-card-title>
        <v-card-text
          >Students were stored successfully, but there are duplicate entries
          found.</v-card-text
        >
        <v-card-text>
          <v-card-title class="ml-0 pl-0">Duplicate Entries</v-card-title>
          <v-card-subtitle class="ml-0 pl-0 text-danger"
            >*These students were not inserted.</v-card-subtitle
          >
          <v-simple-table dense height="300px">
            <template v-slot:default>
              ...
              <tbody>
                <tr v-for="(item, i) in duplicatedDialog.students" :key="i">
                  <td>{{ i + 1 }}</td>
                  <td>{{ item }}</td>
                </tr>
              </tbody>
    ...
    <!--  Edit student Dialog  -->
    <v-dialog v-model="editStudentDialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
              ...
              <v-col cols="12">
                <div class="input-group mb-4">
                  ...
                  <input
                    ...
                    type="date"
                    :class="{ 'is-invalid': editForm.errors.get('birthdate') }"
                    class="form-control"
                    placeholder="Birthdate"
                    aria-label="Birthdate"
                  />
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="editStudentDialog = false">
            Close
          </v-btn>
          <v-btn color="blue darken-1" text @click="handleEditSave">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    async resetPassword() {
      ...
      await axios
        .post(`/api/resetPassword/${this.resetUserID}`, {
    ...
    handleEditSave() {
      this.editForm
        .patch(api.updateStudent(this.editForm.id))
        .then((r) => {
          this.fetchStudents();
          this.editForm.reset();
    ...
    deleteStudent() {
      api
        .destroyStudent(this.selectedUserID)
        .then((r) => {
          this.fetchStudents();
