<template>
  <div data-app>
              ...
              >
                <template v-slot:item.action="{ item }">
                  <v-btn
                    x-small
                    class="bg-danger"
                    @click="handleRemoveSubject(item, 0)"
                  >
    ...
    <div class="row">
      <v-col>
        <v-card>
          ...
          <v-data-table
            :headers="headers.filter((e) => e.value !== 'remarks')"
            :items="subjects"
            :search="search"
            height="500px"
            :footer-props="{
          ...
          >
            <template v-slot:item.action="{ item }">
              <v-btn
                x-small
                class="bg-success"
                @click="handleAddSubjectDialog(item, 1)"
              >
                <i class="fas fa-folder mr-2"> </i>
                Add
              </v-btn>
    ...
    <v-dialog v-model="remarksDialog" max-width="400">
      <v-card>
        <v-card-title class="headline"> ENTER SECTION </v-card-title>
        <v-card-text>
          <input
            type="text"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="updateSubject">
            Confirm
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    updateSubject() {
      ...
      api
        .updateSubjectStatus(
          { status: this.selectedStatus, remarks },
          this.selectedItem.subject_id
        )
        .then((r) => {
