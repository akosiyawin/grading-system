...
@section('content')
    <v-fab-transition>
        <v-btn absolute class="v-btn--example mt-5 bg-success" dark fab onClick="printNow()" right x-small top>
            <v-icon>mdi-printer-pos</v-icon>
        </v-btn>
    </v-fab-transition>
    <v-fab-transition>
        <v-btn absolute class="v-btn--example mt-5 bg-danger" dark fab href="/handler" right style="top: 30px" x-small top>
            <v-icon>mdi-arrow-left-bold</v-icon>
        </v-btn>
    </v-fab-transition>

    <print-grade year="{{ $yearTitle }}" :data="{{ $data }}" :user="{{ $info }}"
        :grade="{{ $totalGrade }}" units="{{ $totalUnits }}"></print-grade>
@endsection
